...
import { ABOUT_STATS, ABOUT_SECTIONS, ABOUT_LINKS } from '../data.js';
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: 'clamp(34px,5vw,62px) clamp(16px,3.4vw,32px) 190px', display: 'flex', flexDirection: 'column', gap: 'clamp(22px,3vw,32px)' }}>
      ...
      <div style={{
        ...
        <h2 style={{ margin: 0, fontFamily: "'Cormorant Garamond',serif", fontWeight: 500, fontSize: 'clamp(24px,2.9vw,32px)', color: '#F6E9D0' }}>Reading and credits</h2>
        <p style={{ margin: 0, fontSize: 14.5, lineHeight: 1.7, color: 'rgba(246,236,214,.76)' }}>Drop your own URLs in — sources, music credits, the illustration, the repo.</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {ABOUT_LINKS.map((l, idx) => (
            <a key={idx} href="#" className="gc-link-row" style={{
              display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 18, padding: '15px 18px',
              ...
              <span style={{ color: 'rgba(246,236,214,.95)', fontSize: 15.5 }}>{l.label}</span>
              <span style={{ color: 'rgba(246,236,214,.55)', fontSize: 11, letterSpacing: '.16em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>{l.note}</span>
            </a>
          ))}
        </div>
        <span style={{ color: 'rgba(246,236,214,.6)', fontSize: 12.5, marginTop: 4 }}>Made with Ganesh Bhakthi · 2026 · Akash Benki</span>
      </div>
    </div>
  );
}
